...
def process_batch(video_paths, batch_size, num_frames=16):
    ...
    for i in range(num):
        ...
        try:
            label_parts = video_paths[i].split("\\")
            label = int(label_parts[-2]) if len(label_parts) >= 2 else -1
        except Exception as e:
            print(f"Error processing path {video_paths[i]}: {e}")
            continue
        labels[i] = label
    return batch, labels
...
def main():
    ...
    model_name="conv3d"
    batch_size=32
    load_weights_name="results\Cnn.hdf5"
    ...
    if model_name == "c3d":
        model = c3d_model(batch_size=batch_size)
    else:
    ...
    model.load_weights("results\Cnn.hdf5")
