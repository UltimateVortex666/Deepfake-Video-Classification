...
if __name__ == "__main__":
	...
	test_label = np.load("train_label_facenet_embeddings.npy")
	...
	for i in videos:
		...
		# After getting predictions from the classifier
		...
		# Convert the filtered numeric values to float32
		y_pred = np.array(numeric_values, dtype=np.float32)
		...
		# Calculate the mean of y_pred
		if len(y_pred) > 0:
			pred_mean = np.nanmean(y_pred, axis=0)  # Use np.nanmean to handle NaN values
		else:
			pred_mean = np.nan  # Set to NaN if y_pred is empty

		# Calculate the mean of y_probabs
		if len(y_probabs) > 0:
			probab_mean = np.nanmean(y_probabs, axis=0)  # Use np.nanmean to handle NaN values
		else:
			probab_mean = np.nan  # Set to NaN if y_probabs is empty
	...
	for label in test_label:
		try:
			test_label_numeric.append(int(label))
		except ValueError:
			pass

	# Convert the filtered numeric values to a numpy array of integers
	test_label_binary = np.array(test_label_numeric, dtype=int)
	
	fpr, tpr, threshold = roc_curve(test_label_binary, y_probabilities[:, 1])
	fnr = 1 - tpr
