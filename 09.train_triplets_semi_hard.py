...
if __name__ == "__main__":
	...
	x_train, x_test, y_train, y_test = train_test_split(train_data, train_label, test_size=0.1, stratify=train_label, random_state=42)
	# Assuming x_train has shape (batch_size, 1, 512), reshape it to (batch_size, 512)
	x_train = x_train.reshape(x_train.shape[0], -1)

	# Assuming x_test has shape (batch_size, 1, 512), reshape it to (batch_size, 512)
	...
	# Now, the shape of x_train and x_test should be (batch_size, 512), matching the expected input shape of the model
	...
	if train_flag == "True":
		...
		# model.summary()
		scatter(model, to_file='model.png', show_shapes=True, show_layer_names=True)
		...
		model_tr = load_model("triplets_semi_hard_sacred_model.hdf5",
		...
		y_train_encoded = label_encoder.fit_transform(y_train[:512])
		
		print("Shapes of extended data and labels:", x_train_before.shape, y_train.shape)
		...
		if embed=="TSNE":
			...
			scatter(train_tsne_embeds_before_train, y_train_encoded, "Training Data Before TNN")
			scatter(train_tsne_embeds_after_train, y_train_encoded, "Training Data After TNN")
			...
			decomposed_embeddings = pca.fit_transform(x_train_before)
